...
placeInjection(unsigned int hpos)
{
  ...
  for (; (*c); c++)
    printf("\033[23;%df\033[1;37;40m%c\033[00m", hpos + (c - &injector[0]) + 1, *c);
}
...
initList(void)
{
  sect *list = (sect *)vrna_alloc(sizeof(sect));
           ...
           int      stage)
{
  ...
  list  = (sect *)vrna_alloc((length + 1) * sizeof(sect));
  xmax  = ymax = 0;
  xmin  = ymin = INF;
        ...
        if (!cnt2) {
          ...
          while ((*d->probe).ml >= 0) {
            if ((*(d->probe)).j == (20)) {
              memmove(((d->probe)), ((d->probe)) + 1,
                      (((d->probe_n)) - (((d->probe)) - (t))) * sizeof(sect));
              (t)           = (sect *)vrna_realloc((t), sizeof(sect) * ((d->probe_n)));
              ((d->probe))  = (t);
              ((d->probe_n))--;
              continue;
            }
          ...
          while ((*d->reactive).ml >= 0) {
            if ((*(d->reactive)).j == (0)) {
              memmove(((d->reactive)), ((d->reactive)) + 1,
                      (((d->reactive_n)) - (((d->reactive)) - (t))) * sizeof(sect));
              (t)             = (sect *)vrna_realloc((t), sizeof(sect) * ((d->reactive_n)));
              ((d->reactive)) = (t);
              ((d->reactive_n))--;
              continue;
            }
            ...
            if (insideBB(i, j, inv_bb)) {
              ...
              while ((*(d->struct_list)).ml >= 0) {
                ...
                if ((i == p) && (j == q)) {
                  ...
                  (d->reactive) = (sect *)vrna_realloc((d->reactive),
                                                       sizeof(sect) *
                                                       ((d->reactive_n) + 2));
                  ...
                  memmove((d->probe), (d->probe) + 1,
                          ((d->probe_n) - ((d->probe) - (ts1))) * sizeof(sect));
                  (ts1)       = (sect *)vrna_realloc((ts1), sizeof(sect) * (d->probe_n));
                  (d->probe)  = (ts1);
                  (d->probe_n)--;
                  ...
                  memmove((d->struct_list), (d->struct_list) + 1,
                          ((d->length) - ((d->struct_list) - (ts2))) * sizeof(sect));
                  (ts2)             = (sect *)vrna_realloc((ts2), sizeof(sect) * (d->length));
                  (d->struct_list)  = (ts2);
                  (d->length)--;
          ...
          while ((*(d->reactive)).ml >= 0) {
            ...
            if ((j == 0) && (i >= hpos) && (i < (hpos + injector_len))) {
              ...
              memmove((d->reactive),
                      (d->reactive) + 1,
                      ((d->reactive_n) - ((d->reactive) - (ts1))) * sizeof(sect));
              (ts1)         = (sect *)vrna_realloc((ts1), sizeof(sect) * (d->reactive_n));
              (d->reactive) = (ts1);
              (d->reactive_n)--;
          ...
          if (state == 2) {
            ...
            if (c == 68) {
              ...
              if (d->probe_n < probe_max) {
                (d->probe) =
                  (sect *)vrna_realloc((d->probe), sizeof(sect) * ((d->probe_n) + 2));
